...
void motor_control()
{
    ...
    if (!stop_now)
    {
        ...
        pwm_l_add = MotorPID_Output(&sptr_l, speed_l, (int16)((float)target_speed - level * (float)dspeed_here));
        pwm_r_add = MotorPID_Output(&sptr_r, speed_r, (int16)((float)target_speed + level * (float)dspeed_here));
        ...
        //               oscilloscope_data.data[4] = d_speed;
